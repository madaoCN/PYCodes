...
def dealPersonInfo(info):
    ...
    if re.search(u'培训|毕业|读书|学习|学历', info):
        # print '找到学历匹配项'
        # print info
        return True
    else:
        return False
def dealNativePlaceInfo(info):
    '''判断是否带有籍贯信息'''
    if re.search(u'.+?生', info) and re.search(u'.+?人', info):
        # print info
        return True
    else:
        return False

def dealPositionInfo(info):
    '''判断是否带有职位信息'''
    print 'nothing'
...
def praseHTML(filePath):
    ...
    with codecs.open(filePath, encoding='utf8') as file:
        ...
        for li in paramDIC['sentence']:
            sentence = ''.join(li)
            if not dealPersonInfo(sentence) and not dealNativePlaceInfo(sentence):
                print sentence
