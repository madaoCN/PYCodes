...
def getUrl(target_url, index):
    ...
    prepare = Request('GET', curentURL).prepare()
...
def praseJsonForOne(response, index):
    ...
    try:
        ...
        print e
    #用户名
    userName = soup.find('div', {'class':'time'}).find('span', {'class':'laiyuan'}).text
    dataArr.append(userName)
    #ID
    dataArr.append(index)
    #阅读数
    try:
        readNum = soup.find('span', {'id':'content_click'}).text
        readNum = readNum.split(' ')[1]
    except Exception, e:
        print e
    dataArr.append(readNum)
    #点赞数
    try:
        zanNum = soup.find('span', {'id':'like_count'}).text
    except Exception, e:
        print e
    dataArr.append(zanNum)

    getNextUrl(index, dataArr)

def getNextUrl(index, dataArr):
    currentURL = HOST_2 % index
    print '################################getNextUrl'

    prepare = Request('GET', currentURL).prepare()
    #try多次
    attempts = 0
    success = False
    while attempts < 3 and not success:
...
def writeTofile(dataArr, myDic):
    ...
    path = os.path.expanduser(r'~/Desktop/data/data30000-40000.txt')
    print path
    f = open(path, "a")
    if dataArr:
        for rag in dataArr:
...
pool = Pool(5)
# myCookie = getCookies()
for index in range(30000, 40000):
